...
const createCalendarResources = async( authorizedClient, pgClient ) => {
    ...
    if( msOrg && users ) {
        try {
            ...
            const roomA2_2 = await createRoom( authorizedClient, orgId, floorA2.id, "Athena", 17, "athena@microsoft.evoko.dev" )
            const roomA1_2 = await createRoom( authorizedClient, orgId, floorA1.id, "Demeter", 8, "demeter@microsoft.evoko.dev" )
            const roomA2_3 = await createRoom( authorizedClient, orgId, floorA2.id, "Poseidon", 10, "poseidon@microsoft.evoko.dev" )
            const roomA2_4 = await createRoom( authorizedClient, orgId, floorA2.id, "Zeus", 4, "zeus@microsoft.evoko.dev" )
